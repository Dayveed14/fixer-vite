...
export default function DIYSection() {
  ...
  const thumbOf = (item) => (tab === "articles" ? item.hero_image : item.thumbnail_url);
  const linkTo = (item) => (tab === "articles" ? `/userarticle/${item.slug}` : `/uservideo/${item.slug}`);
            ...
            {related.map(item => (
              <div className="diy__related-item" key={item.id}>
                <div className="diy__related-img-wrap">
                  <img src={thumbOf(item)} alt={item.title} className="diy__related-img" />
                ...
                <div>
                  <Link to={linkTo(item)} className="diy__related-title">{item.title}</Link>
