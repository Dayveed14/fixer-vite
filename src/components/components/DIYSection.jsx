...
const DEFAULT_AVATAR = "https://i.pravatar.cc/40?img=15";
...
export default function DIYSection() {
      ...
      try {
        const endpoint =
          tab === "articles"
            ...
            : "https://fixer-backend-7mng.onrender.com/api/diy-videos/published";
  ...
  // Articles store their image on hero_image; videos store it on
  // thumbnail_url (auto-generated by Cloudinary from the video itself).
  const thumbOf = (item) =>
    tab === "articles" ? item.hero_image : item.thumbnail_url;
  const linkTo = (item) =>
    tab === "articles"
      ? `/userarticle/${item.slug}`
      : `/uservideo/${item.slug}`;
      ...
      <div className="diy__inner">
        <div className="diy__top">
          <div>
            ...
            <p className="diy__sub">
              Fix your gadgets yourself with quick guides, videos, and expert
              tips—save money, solve issues, and extend device life easily.
            </p>
          </div>
          <div className="diy__tabs">
            <button
              className={`diy__tab ${tab === "articles" ? "active" : ""}`}
              onClick={() => setTab("articles")}
              title="Articles">
              <IconBook />
            </button>
            <button
              className={`diy__tab ${tab === "videos" ? "active" : ""}`}
          ...
          <Link to={linkTo(featured)} className="diy__featured">
            ...
            <div className="diy__meta">
              <img
                ...
                className="diy__avatar"
              />
              <span className="diy__author">
              ...
              <div className="diy__related-item" key={item.id}>
                <div className="diy__related-img-wrap">
                  <img
                    src={thumbOf(item)}
                    alt={item.title}
                    className="diy__related-img"
                  />
                  {tab === "videos" && (
                    <div className="diy__play-btn diy__play-btn--sm">
                      <svg
                        width="12"
                        height="12"
                        viewBox="0 0 24 24"
                ...
                <div>
                  <Link to={linkTo(item)} className="diy__related-title">
                    {item.title}
                  </Link>
                  <div className="diy__meta diy__meta--sm">
                    <img
                      src={item.avatar}
                      alt=""
                      className="diy__avatar diy__avatar--sm"
                    />
                    <span className="diy__author">
